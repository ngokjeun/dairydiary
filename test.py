...
class OptimisticSMP:
    ...
    def prepare_curves(self):
        ...
        forward_curves_filtered['Period'] = forward_curves_filtered['Period'].apply(
            _calc_future_date)
        forward_curves_filtered = forward_curves_filtered[[
            'Period', 'Price', 'Carry', 'Input Adjustment', 'Market']]
        ...
        self.sales_data['Date'] = pd.to_datetime(
            self.sales_data['Day of DateDeliverySchemeThru'], format='%Y/%m/%d')
    ...
    def process_data(self, data_frame, item_name='Skimmed Milk Powder'):
        ...
        # Ensure comparison is date to date, not datetime to date
    ...
    def rename_city_region(self):
        self.sales_df['PlaceOfDelivery'] = pd.merge(self.sales_df, self.sales_data[[
    ...
    def get_allocations_df(self):
        self.stocks_df['Region'] = self.stocks_df['Region'].apply(
            lambda x: self.country_region_mappings.get(x, x))
    ...
    def test_unique_values(self):
        # Step 1: Check unique values before any processing
        print("Before any processing:")
        print(self.sales_data['PlaceOfDelivery'].unique())

        # Step 2: Check unique values after each operation that modifies it
        ...
        # After preparing the curves
        self.prepare_curves()
        print("After preparing the curves:")
        print(self.sales_data['PlaceOfDelivery'].unique())

        # After preparing the sales
        self.prepare_sales()
        print("After preparing the sales:")
        print(self.sales_data['PlaceOfDelivery'].unique())

        # After renaming city region
        self.rename_city_region()
        print("After renaming city region:")
        print(self.sales_data['PlaceOfDelivery'].unique())

        # After preparing for optimization
        self.prepare_for_optimization()
        print("After preparing for optimization:")
        print(self.sales_data['PlaceOfDelivery'].unique())
